...
module Refinery
  class Page < Core::BaseModel
    ...
    def path(options = {}) # Changes for 1.9.3 compatibility
      # Override default options with any supplied.
      options = {:path_separator => ' - ', :ancestors_first => true}.merge(options)

      return title if root?

      chain = options[:ancestors_first] ? self_and_ancestors : self_and_ancestors.reverse
      chain.map(&:title).join(options[:path_separator])
    end
